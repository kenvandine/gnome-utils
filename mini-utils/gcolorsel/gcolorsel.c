/* Gnome Color Browser. By Tim P. Gerla. <timg@means.net> */
...
int main(int argc, char **argv)
{
	...
	/* Create entries */
	hbox = gtk_hbox_new(TRUE, 2);
	label = gtk_label_new(N_("Color Name:"));
	...
	hbox = gtk_hbox_new(TRUE, 2);
	label = gtk_label_new(N_("Color Values:"));
	...
	button = gnome_stock_or_ordinary_button(GNOME_STOCK_BUTTON_CLOSE);
	gtk_signal_connect(GTK_OBJECT(button), "clicked",
        	GTK_SIGNAL_FUNC(delete_event), NULL);
	gtk_box_pack_start_defaults(GTK_BOX(hbox), button);
	gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 2);
...
void load_rgb(GtkWidget *clist)
{
	...
	if(!file)
	{
		gtk_widget_show(gnome_message_box_new(
			N_("Error, cannot open /usr/X11R6/lib/X11/rgb.txt.\n"), "error",
